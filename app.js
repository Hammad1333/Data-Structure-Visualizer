...
const heapViz = {
  ...
  async extract() {
    if (this.animating || this.data.length === 0) return;
    this.animating = true;
  ...
  // ADD THIS METHOD to your heapViz object (after the draw() method):
...
const avlViz = {
  ...
  logs: [],
  maxLogs: 15,
  nodePositions: new Map(), // ADD THIS
  max(a, b) {
    return a > b ? a : b;
  },
  ...
  drawNode(ctx, node, x, y, offset) {
    ...
    // Draw info box below node (like in your image)
    const boxY = y + 28;
    const boxWidth = 50;
    const boxHeight = 16;
...
const hashViz = {
  ...
  animating: false, // ADD THIS
  animatedNodes: new Map(), // ADD THIS
  hashFunction(key) {
  ...
  draw() {
    ...
    this.table.forEach((head, i) => {
      ...
      while (current) {
        ...
        if (current.next) {
          // This is a simplified representation of the link
          ctx.beginPath();
          ctx.moveTo(currentX + boxW, y + boxH / 2);
          ctx.lineTo(currentX + boxW + 20, y + boxH / 2);
          ctx.stroke();
          // Arrowhead logic would go here
        }

        currentX += boxW + 20; // Move x position for the next node in the chain
        current = current.next;
      }
      // Draw Empty Box if chain is null (optional, but helps visualize empty slots)
      if (!head) {
...
// Define these BEFORE the loop to prevent reference error
const radius = 3.5;
...
dataStructures.forEach((ds, i) => {
  ...
  // Draw icon/symbol based on data structure
  ctx.textAlign = "center";
  ctx.textBaseline = "middle";
  ...
  // Create plane instead of sphere
  ...
  spheres.push(plane); // Still using 'spheres' array name
  scene.add(plane);
});
...
// Keyboard Rotation
// Keyboard Rotation
// Keyboard Rotation AND Enter Selection
...
// Function to find the closest sphere to the front (closest to user)
function getClosestSphereToFront() {
  let mostCentered = null;
  let minDistance = Infinity;

  spheres.forEach((sphere) => {
    // Calculate distance from center of screen (x should be close to 0)
    // and should be in front (positive z value)
    const distanceFromCenter = Math.abs(sphere.position.x);

    // Only consider spheres that are reasonably in front
